...
namespace Moq
{
	internal class MethodCall<TProperty> : MethodCall, IExpectSetter<TProperty>, IThrowsIfResult<TProperty>
	{
		...
		public override bool Matches(IInvocation call)
		{
			...
			if (condition != null)
				return call.Method == method && 
					condition((TProperty)call.Arguments[0]);
			else
				return call.Method == method;
		}
		...
		public new IThrowsIfResult<TProperty> Throws(Exception exception)
		{
			base.Throws(exception);
			return this;
		}

		public new IThrowsIfResult<TProperty> Throws<TException>() where TException : Exception, new()
		{
			base.Throws<TException>();
			return this;
		}

		public IThrowsResult If(Predicate<TProperty> condition)
		{
