...
namespace Moq.Tests
{
	public class MockFixture
	{
		...
		[Fact]
		public void ThrowsOnSetOnlyIfMatchesCondition()
		{
			var mock = new Mock<IFoo>();

			mock.ExpectSet(x => x.ValueProperty).Throws<ArgumentException>().If(i => i < 5);
			mock.ExpectSet(x => x.ValueProperty).Throws<InvalidOperationException>().If(i => i > 15);

			mock.Object.ValueProperty = 10;

			Assert.Throws<ArgumentException>(() => mock.Object.ValueProperty = 1);
			Assert.Throws<InvalidOperationException>(() => mock.Object.ValueProperty = 20);
		}
