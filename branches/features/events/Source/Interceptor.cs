...
namespace Moq
{
	...
	internal class Interceptor : MarshalByRefObject, IInterceptor
	{
		...
		public void Intercept(IInvocation invocation)
		{
			...
			{
				var delegateType = invocation.Method.GetParameters().First().ParameterType;
				if (delegateType.GetGenericTypeDefinition() != typeof(EventHandler<>))
					// TODO: test
					throw new NotSupportedException();

				var delegateInstance = (Delegate)invocation.Arguments[0];
				var mockEvent = delegateInstance.Target as MockEvent;

				if (mockEvent != null)
				{
					// TODO: validate we can get the event.
					mockEvent.Event = targetType.GetEvent(invocation.Method.Name.Replace("add_", ""));
				}
				else
				{
					mockEvent.AddHandler((Delegate)invocation.Arguments[0]);
				}
