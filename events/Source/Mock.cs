...
namespace Moq
{
	...
	public class Mock
	{
		...
		List<MockEvent> handlers = new List<MockEvent>();

		internal MockEvent FindHandler(Type argsType)
		{
			return handlers.FirstOrDefault(x => x.ArgsType == argsType);
		}

		internal MockEvent<TEventArgs> CreateHandler<TEventArgs>() where TEventArgs : EventArgs
		{
			var handler = (MockEvent<TEventArgs>)FindHandler(typeof(TEventArgs));

			if (handler == null)
			{
				handler = new MockEvent<TEventArgs>();
